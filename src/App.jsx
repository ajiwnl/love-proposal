...
const moveNoButton = () => {
  ...
  const fakeApi = async (url, payload) => {
    ...
    if (url === "/api/relationship/confess") {
      if (payload.name.trim().toLowerCase() === "jessa") {
        return {
          success: true,
          message: "💌 Will you be my forever, Jessa?",
        };
      } else {
        return {
          success: false,
          message: "😢 Sorry... my heart only belongs to Jessa.",
        };
      }
    }
    ...
    if (url === "/api/relationship/commit" && payload.accepted) {
      return {
        success: true,
        message: "💖 You and Jessa are now officially together! 💍✨",
      };
    }
  ...
  const acceptProposal = async () => {
    ...
    const response = await fakeApi("/api/relationship/commit", { accepted: true });
  ...
  return (
  <div className="relative flex items-center justify-center min-h-screen w-screen bg-gradient-to-br from-pink-200 via-rose-300 to-rose-500 text-white text-center overflow-hidden">
    ...
    <motion.div
      ...
      className="relative flex flex-col items-center justify-center bg-white/20 backdrop-blur-lg p-8 rounded-3xl shadow-2xl w-80 sm:w-96 text-center"
    >
      {stage === "confess" && (
        <>
